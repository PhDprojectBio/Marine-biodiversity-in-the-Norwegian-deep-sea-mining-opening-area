...
if (!dir.exists("data/bathy")) {
  ...
  for(i in 1:length(urls)){
    ...
    download.file(url=urls[i],
                  destfile=paste0("data/bathy/tile_",i,"2024.tif.zip"))
    route <- unzip(zipfile = paste0("data/bathy/tile_",i,"2024.tif.zip"), exdir = "data/bathy")
    
  }}

# Get rasters
rasterList <- list.files("data/bathy",pattern = "_2024.tif", full.names = TRUE)
