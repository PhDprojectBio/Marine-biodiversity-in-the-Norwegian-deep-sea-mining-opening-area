...
for(i in 1:length(all24)){ #length(all24)
...
for (b in 1:length(all24_mkd)){ 
  ...
  taxon <- read.delim("data/taxon.txt") %>% filter(acceptedNameUsage != "") # to get accepted names and the names where they come from (i.e Lophelia) # filter(taxonomicStatus == "accepted") only has accepted names, but not the species that has been re-assigned from their original designation.
  speciesprofile <- read.delim("data/speciesprofile.txt")
  ...
  one <- readxl::read_excel("data/one.xlsx", col_names = FALSE)
